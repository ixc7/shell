...
format_menu () {
	if [ "$is_ext_menu" -eq 0 ]; then
		...
		# 09/22: MENU_COMMAND:
		#        -> IMPORTANT!
		#        -> CHANGES THE MAIN DISPLAY COMMAND.

		menu_command='fzf \
		  -m --bind change:top --tabstop=1 --layout=reverse-list \
		  --delimiter="$tab_space" --nth=1,2 --expect="$shortcuts" \
		  --cycle --border=sharp --margin 5% \
		  --preview "cat .ytfzf__temp | grep {} | sed -e \"s/|/\\\n/g\"" \
		  --preview-window="top:35%:border-sharp:wrap:cycle" \
		  --no-info \
...
preview_img () {
  ...
  # 09/22: ADDED TIMG HERE!!!!!
	...
	case "$YTFZF_THUMB_DISP_METHOD" in
		...
		jp2a --size="${thumb_width}x$((thumb_height / 2))" --colors "$IMAGE" ;;
	    jp2a-8)
		printf "\n"
      ...
      timg)
	  printf "\n" 
    timg -g"${thumb_width}x${thumb_height}" -W -pq "$IMAGE";;
	    custom)
...
video_menu () {
  ...
  # 09/22: TRYING TO DISPLAY PREVIEWS RIGHT.
  # IMPORTANT.
	echo "$*" > .ytfzf__temp
...
verify_thumb_disp_method () {
	...
	ueberzug|jp2a-grey|jp2a-gray|jp2a|jp2a-4|jp2a-8|catimg|custom|chafa|chafa-grey|chafa-gray|chafa-4|chafa-8|timg) ;;
	*)
