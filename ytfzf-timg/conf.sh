...
# thumb_disp_method="jp2a"

# 09/22: IMPORTANT. TRYING TO INTEGRATE TIMG.
thumb_disp_method="timg"
...
# video_player="mpv --no-terminal --msg-level=no"
video_player="mpv --no-video --quiet"
...
#(YTFZF_AUDIO_PLAYER)
audio_player="mpv --no-video --quiet"
...
# 09/22 BROKEN (?) LEAVE ON 0 FOR NOW...
# prints usage: date ...
sort_videos_data=1
...
video_info_text() {
	...
	# echo "$date $duration $title $channel $views $shorturl"
	# echo "$title"
...
data_sort_key () {
    ...
    # 09/22: THIS IS WRONG. KEEPS PRINTING OUT -> usage: date ...
